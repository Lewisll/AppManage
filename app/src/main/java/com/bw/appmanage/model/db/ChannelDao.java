...
public class ChannelDao implements ChannelDaoInface {
	...
	@Override
	public List<String> listCacheData(String selection, String[] selectionArgs) {
		...
		try {
			...
			for (int i = 0; i < count; i++) {
				if (cursor.getString(cursor.getColumnIndex("name")).equals(selectionArgs[0])){
					return null;
				}
			}
			list.add(selectionArgs[0]);
		} catch (Exception e) {
		} finally {
			if (cursor!=null){
				cursor.close();
	...
	/**查询重复*/
	@Override
	public List<String> listAllCache(String selection, String[] selectionArgs) {
		List<String> list = new ArrayList<String>();
		SQLiteDatabase database = null;
		Cursor cursor = null;
		try {
	...
	/**删除某个字段*/
	@Override
	public boolean deleteOtherCache(String selection, String[] selectionArgs) {
		List<String> list = new ArrayList<String>();
		SQLiteDatabase database = null;
		boolean flag = false;
		int count = 0;
		try {
			database = helper.getReadableDatabase();
			count = database.delete(SQLHelper.TABLE_CHANNEL, selection,selectionArgs);
			flag = (count > 0 ? true : false);
		} catch (Exception e) {
		} finally {
			if (database != null) {
